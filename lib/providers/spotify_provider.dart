...
import '/models/album_model.dart'; // Assurez-vous que ce chemin est correct
import '/models/artiste_model.dart';
...
class ApiService {
  ...
  Future<void> _fetchToken() async {
    _accessToken =
        "BQATA_7jL5rFccaooyFvl3-U63lupq6IUvXmst1cMxDMWDdU0RI8u9DBoSJXC-bDPiMKN0POm_jyUgdSkEOMQe9RAxdMtTEqCtNSvFFZhVPuhwPfpRg"; // Remplacez 'Votre_Token' par votre token d'authentification Spotify
  }
  ...
  Future<List<Album>> searchAlbums(String query) async {
    final url = Uri.parse('https://api.spotify.com/v1/search?type=album&market=FR&q=$query');
    ...
    if (response.statusCode == 200) {
      ...
      List<Album> albums = albumsJson.map((json) => Album.fromJson(json)).toList();
      return albums;
    } else {
